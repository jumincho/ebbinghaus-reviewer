...
<img src="docs/screenshots/today.png" width="49%" alt="오늘(Today) 페이지: 통계와 복습할 항목">
<img src="docs/screenshots/review.png" width="49%" alt="네 가지 평가 버튼이 있는 다크 모드의 복습(Review) 페이지">

</div>

## 개요

무언가를 공부했다면 바로 기록해 둡니다. Ebbinghaus Reviewer는 그 내용을 막 잊어버리려 할 때쯤 다시 복습하도록 불러옵니다. 처음에는 **10분** 뒤, 그다음에는 **1일**, **1주**, **1개월** 뒤에 복습하게 되며, 날마다 복습할 항목을 알려 줍니다. 복습할 때마다 다시(*again*), 어려움(*hard*), 좋음(*good*), 쉬움(*easy*) 중 하나로 평가하면 일정이 그에 맞게 조정됩니다.
...
명령줄만 쓰려면 `[web]`을 빼면 됩니다. `demo` 명령은 빈 컬렉션에서만 실행됩니다. 별도 파일에서 시험해 보려면 `--db demo.sqlite3`를 지정하면 됩니다.
...
| `ebbinghaus demo` | 빈 데이터베이스에 샘플 컬렉션을 불러옵니다. |
...
`ebbinghaus serve`는 명령줄과 같은 SQLite 파일을 사용하는 로컬 웹 앱을 실행합니다. 그래서 터미널에서 기록하고 브라우저에서 복습할 수 있습니다. 페이지는 통계와 복습할 항목을 보여 주고 빠른 추가도 할 수 있는 **오늘**(*Today*), 카드를 한 장씩 보여 주고 메모는 펼치기 전까지 숨겨 두는 **복습**(*Review*), 과목별로 필터링할 수 있는 **항목**(*Items*), 일정·기록·편집·다시 시작을 다루는 **항목 페이지**, 그리고 **일정표**(*Agenda*)로 이루어져 있습니다.

<p align="center">
  <img src="docs/screenshots/item.png" width="80%" alt="일정 정보와 망각 곡선 차트가 표시된 항목 페이지">
</p>
...
SM-2의 간격은 1일, 6일 순이고, 그다음부터는 이전 간격 × 용이도 계수를 올림한 값입니다. 회상 확률 추정치는 항목의 복습 기한이 되는 순간 정확히 90%에 이르는 지수형 망각 곡선을 가정합니다. [`docs/algorithm.md`](docs/algorithm.md)에는 전체 규칙과 계산 예시, 참고 문헌이 있고, [`docs/architecture.md`](docs/architecture.md)에서는 계층 구조, 데이터 모델, 시간 처리를 설명합니다.
