...
// Bind() is exposed so NotificationService can populate the popup with an
// item + which slot fired. Highlighting the active slot is done by switching
// Foreground/FontWeight on the matching TextBlock.
//
// Field names (seper, firstl, sndl, thdl, forthl, first, snd, thd, forth,
// subjectlabel, todoname) are taken from TodolistReminder.g.cs. The spelling
// of "seper" (vs. "separator") matches the original.
